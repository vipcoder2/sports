...
import { Send, Smile, Image, Users, MessageCircle, MoreVertical, X, Search, Volume2, VolumeX, LogIn } from 'lucide-react';
...
export function Chat({ matchId, className = '' }: ChatProps) {
  ...
  // Initialize user session with logged-in username
  useEffect(() => {
    if (currentUser) {
      // Use display name or email as username for logged-in users
      const autoUsername = currentUser.displayName || currentUser.email?.split('@')[0] || 'User';
      setUsername(autoUsername);
      userData.current = generateUserData(autoUsername);
      setHasJoined(true);
    }
  }, [currentUser]);
        ...
        <CardContent className="space-y-6">
          <div className="text-center space-y-2">
            <p className="text-sm text-muted-foreground">
              You need to sign in to join the live chat and connect with other fans
            </p>
            <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground/70">
              <MessageCircle className="w-3 h-3" />
              <span>Real-time messaging for authenticated users</span>
            </div>
          </div>
          ...
          <p className="text-xs text-muted-foreground/60 text-center">
            Join thousands of sports fans in real-time discussions
          </p>
        </CardContent>
      </Card>
    );
  ...
  // Chat loads automatically for logged-in users
    ...
    return (
      <Card className={`w-full max-w-md mx-auto glassmorphism ${className}`}>
        <CardHeader className="text-center space-y-4">
          ...
          <CardTitle className="text-lg">Loading Chat...</CardTitle>
        </CardHeader>
        <CardContent className="text-center">
          <p className="text-sm text-muted-foreground">
            Connecting you to the live chat as {currentUser.displayName || currentUser.email?.split('@')[0]}
          </p>
        </CardContent>
      </Card>
    );
