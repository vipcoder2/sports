...
export function StreamSources({ match }: StreamSourcesProps) {
  ...
  const handleCaptchaVerified = () => {
    ...
    // Store verification in sessionStorage for use in stream player
        ...
        <div className="min-h-screen px-3 sm:px-4 py-4">
          ...
          <div className="text-center mb-6">
            <h1 className="text-lg sm:text-2xl md:text-3xl font-bold text-foreground mb-2 leading-tight">
              {parseTeams(match.title).away ? `${parseTeams(match.title).home} vs ${parseTeams(match.title).away}` : match.title}
            </h1>
          ...
          <div className="max-w-md mx-auto">
            <Card className="glassmorphism border-accent/20">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">Stream Protected</h3>
                <p className="text-sm text-muted-foreground">
                  Complete security verification to view available stream sources
                </p>
              </CardContent>
            </Card>
          </div>
                ...
                {source.error ? (
                  ...
                  <div className="space-y-2">
                    {source.streams
                      .slice(
                        0,
                        expandedSources.has(source.source) ? undefined : 12,
                      )
                      .map((stream) => (
                        <Button
                          key={stream.streamNo}
                    ...
                    {source.streams.length > 6 &&
                      !expandedSources.has(source.source) && (
                        <Button
                          ...
                          onClick={() => toggleSourceExpanded(source.source)}
                        >
                          Show {source.streams.length - 12} more streams
