...
export default function MatchDetail() {
  ...
  // Block search engines from indexing match detail pages
  useEffect(() => {
  ...
  console.log('Available matches count:', matches.length);
  ...
  console.log('Match found:', !!currentMatch, currentMatch?.title);
  if (!currentMatch && matches.length > 0) {
    console.log('Available match IDs sample:', matches.slice(0, 10).map(m => ({ id: m.id, title: m.title })));
  }
  ...
  const handleFavoriteToggle = async () => {
    if (currentMatch) {
      await toggleFavorite(currentMatch.id);
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 flex-shrink-0">
                  {currentMatch.teams?.home?.badge && (
                    <img
                      src={`https://streamed.pk/api/images/proxy/${encodeURIComponent(currentMatch.teams.home.badge)}.webp`}
                  ...
                  {currentMatch.teams?.away?.badge && (
                    <img
                      src={`https://streamed.pk/api/images/proxy/${encodeURIComponent(currentMatch.teams.away.badge)}.webp`}
              ...
              <div className="text-center space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-center max-w-sm mx-auto">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setLocation(`/match/${currentMatch.id}/streams`)}
                    className="h-9 text-sm flex-1 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white border-0 animate-pulse shadow-lg shadow-blue-500/25"
          ...
          {/* Social Banners - Strategically placed after stream access */}
          <SocialBanners />
          ...
          {(currentMatch.poster || currentMatch.teams?.home?.badge) && (
            <Card className="glassmorphism border-border overflow-hidden">
              <img
                src={
                  currentMatch.teams?.home?.badge && currentMatch.teams?.away?.badge
                    ? `https://streamed.pk/api/images/poster/${encodeURIComponent(currentMatch.teams.home.badge)}/${encodeURIComponent(currentMatch.teams.away.badge)}.webp`
                    : currentMatch.poster
                    ? `https://streamed.pk/api/images/proxy/${encodeURIComponent(currentMatch.poster)}.webp`
                    : currentMatch.teams?.home?.badge
                    ? `https://streamed.pk/api/images/proxy/${encodeURIComponent(currentMatch.teams.home.badge)}.webp`
                    : ''
                }
