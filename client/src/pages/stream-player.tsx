...
export default function StreamPlayerPage() {
  ...
  const [captchaVerified, setCaptchaVerified] = useState(() => {
    // Check if user came from stream sources page (where captcha was already verified)
    // or if they have a recent verification in sessionStorage
  ...
  // Block search engines from indexing stream pages
  useEffect(() => {
  ...
  const { data: streams = [], isLoading: streamsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const matchSource = currentMatch.sources.find(
        (s) => s.source === params.source,
      );
      if (!matchSource) {
        console.log(
    ...
    enabled: Boolean(params?.source && params?.matchId && currentMatch && captchaVerified),
  });

  const currentStream = streams.find(
    (s: Stream) => s.streamNo.toString() === params?.streamNo?.toString(),
  );
  ...
  useEffect(() => {
    if (currentMatch) {
      ...
      const structuredData = {
        ...
        "@type": "VideoObject",
        name: teams.away
          ? `${teams.home} vs ${teams.away} Live Stream`
          : `${currentMatch.title} Live Stream`,
    ...
    return (
      <div className="min-h-screen bg-[#011412]">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <Alert className="glassmorphism border-destructive">
            <AlertDescription>
              Stream not found or no longer available. Available streams:{" "}
              {streams.length}
              <br />
  ...
  return (
    <CopyrightProtection protectedContent={true}>
      {/* Anti-bot CAPTCHA Overlay for Stream Player */}
      {!captchaVerified && (
        <CaptchaOverlay onVerified={handleCaptchaVerified} />
      )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="text-center mb-4 lg:mb-6">
          <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground mb-2 px-2">
            ...
            <span className="hidden sm:inline">
              {" "}
              Stream {params?.source} {currentStream.streamNo}
            </span>
          </h1>
          <p className="text-sm text-muted-foreground px-4 sm:px-0">
            ...
            <span className="hidden sm:inline">
              {" "}
              Join the stream and chat with others in our live chat!
            </span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          <div className="lg:col-span-3">
            {/* Signup Banner for unauthenticated users */}
            <SignupBanner />
              ...
              <CardContent className="space-y-3 pt-0">
                <div className="grid grid-cols-2 gap-3 sm:space-y-3 sm:grid-cols-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant={currentStream.hd ? "destructive" : "secondary"}
                      className="text-xs"
                    >
                ...
                <div className="space-y-2">
                  <div className="p-2 sm:p-3 rounded-lg bg-primary/10 border border-primary/20">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {streams.slice(0, 12).map((stream: Stream) => (
                        <Button
                          key={stream.streamNo}
                          variant={
                            stream.streamNo === currentStream.streamNo
                              ? "default"
                              : "ghost"
                          }
                          size="sm"
                          ...
                          onClick={() =>
                            setLocation(
                              `/stream/${params?.matchId}/${params?.source}/${stream.streamNo}`,
                            )
                          }
                      ...
                      <div className="space-y-2 md:tablet-grid lg:space-y-2 lg:block">
                        {streams.slice(0, 12).map((stream: Stream) => (
                          <Button
                            ...
                            className="w-full justify-between glassmorphism border-border h-auto py-2"
                            onClick={() =>
                              setLocation(
                                `/stream/${params?.matchId}/${params?.source}/${stream.streamNo}`,
                              )
